...
export const Route = createFileRoute("/categories/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: `${cat?.title ?? "Catégorie"} — Victorious` },
        {
          name: "description",
          content: cat?.description ?? "Catégorie Victorious",
        },
        { property: "og:title", content: `${cat?.title} — Victorious` },
        { property: "og:description", content: cat?.tagline ?? "" },
        ...(cat?.image ? [{ property: "og:image", content: cat.image }] : []),
      ],
    };
  },
  notFoundComponent: () => (
  ...
  errorComponent: () => (
    <div className="grid min-h-[60vh] place-items-center bg-obsidian px-6 pt-32 text-ivory">
...
function CategoryDetail() {
  const { cat } = Route.useLoaderData() as { cat: import("@/content/categories").Category };
    ...
    <>
      ...
      <Section>
        <div className="grid gap-16 lg:grid-cols-[1.3fr_1fr]">
          <div>
            <h2 className="gold-rule font-display text-3xl text-ivory">
              Description
            </h2>
          ...
          <aside className="lg:sticky lg:top-32 lg:self-start">
            <div className="aspect-[4/5] overflow-hidden shadow-frame">
              <img
                src={cat.image}
                alt={cat.title}
                loading="lazy"
