...
export async function createTicketBundlePdf(bundle: TicketBundle): Promise<Uint8Array> {
  ...
  for (const [index, ticket] of bundle.tickets.entries()) {
    ...
    page.drawText("LA NUIT DE L'EXCELLENCE", {
      x: 43,
      y: A4.height - 98,
      size: 9,
      font: sans,
    ...
    page.drawText("BILLET NOMINATIF", {
      x: 72,
      y: 606,
      size: 10,
      font: sans,
    ...
    page.drawText(bundle.reference, {
      x: 72,
      y: 522,
      size: 11,
      font: sans,
...
export async function downloadTicketBundlePdf(bundle: TicketBundle) {
  const bytes = await createTicketBundlePdf(bundle);
  ...
  link.href = url;
  link.download = `victorious-2026-${bundle.reference.toLowerCase()}.pdf`;
